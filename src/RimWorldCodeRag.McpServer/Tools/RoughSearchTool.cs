...
public sealed class RoughSearchTool : ITool, IDisposable
{
    ...
    public string Description =>
        "Search for RimWorld code symbols and XML definitions using natural language queries. " +
        "Returns a list of matching item names with metadata. " +
        "IMPORTANT: Use the 'kind' parameter to filter results - set kind='xml' or kind='def' when searching for XML definitions/Defs, " +
        "set kind='csharp' or kind='cs' when searching for C# code. This dramatically improves result relevance. " +
        "Use get_item tool afterwards to retrieve full source code for any interesting results. " +
        "If search doesn't return relevant results, try simplifying your query to focus on essential keywords only."+
        "Using simple words without dashes or underscores often yields better results.";
        ...
        var schema = new
        {
            ...
            properties = new
            {
                ...
                kind = new
                {
                    ...
                    description = "RECOMMENDED: Filter by type. Use 'xml' or 'def' when searching for XML definitions/ThingDefs/items. Use 'csharp' or 'cs' when searching for C# code/classes/methods. Omit only when you need both types."
                },
                max_results = new
                {
                    type = "integer",
    ...
    public async Task<object> ExecuteAsync(JsonElement arguments)
    {
        ...
        // Get GraphQuerier for node ID lookup
        var graphQuerier = _graphQuerier.Value;
        ...
        var response = new
        {
            ...
            {
                symbolId = r.SymbolId,
                nodeId = graphQuerier.GetNodeId(r.SymbolId),
